...
export default {
  ...
  methods: {
    ...
    async fetchContacts () {
      try {
        const response = await axios.get ('http://localhost:8081/get-contacts')
    ...
    async createContact (contact) {
      try {
        ...
        await axios.post ('http://localhost:8081/create-contact', {
          name: contact.name,
          surname: contact.surname,
          patronymic: contact.patronymic,
          phone: contact.phone
        })
        .then (this.fetchContacts (), this.AlertSuccess ())
      } catch (error) {
        this.AlertDanger ();
      }
    },
    ...
    async editContact (contact) {
      try {
        ...
        await axios.post (`http://localhost:8081/edit-contact/${contactEdit}`, {
          name: contact.name,
          phone: contact.phone,
          surname: contact.surname,
          patronymic: contact.patronymic,
        }).then (this.AlertSuccess (), this.fetchContacts ())
      } catch (error) {
        this.AlertDanger ();
      }
    },
    ...
    async removeContact (contact) {
      try {
        ...
        await axios.delete (`http://localhost:8081/delete-contact/${contactRemoving}`)
        .then (
          this.contacts = this.contacts.filter(cont => cont._id !== contact._id), 
          this.AlertSuccess ()
        )
      } catch (error) {
        this.AlertDanger ();
